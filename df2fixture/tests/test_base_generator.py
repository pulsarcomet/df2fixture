...
def load_data_compressible() -> pd.DataFrame:
    df = load_data()
    df['name'] = 'John'
    df['job'] = 'qa'
    return df
...
class Test_BaseGenerator():
    ...
    @pytest.fixture
    def config_compressible(self) -> list:
        return [
            dict(
                func=load_data_compressible,
                kwargs={
                },
                obfuscators={
                    'id': lambda x: x+1,
    ...
    def test_generate_compress_column(self, config_compressible: dict):
        ...
        def __fixture__case1_load_data_compressible() -> pd.DataFrame:
            df: pd.DataFrame = pd.DataFrame(columns=['id', 'name', 'job'])
            df['id'] = [33, 44]
            df['name'] = 'abc John'
            df['job'] = 'xyz qa'
